"""CLARA database — SQLite persistence with optional PostgreSQL support.

Thread-safe SQLite with WAL mode.  PBKDF2-HMAC-SHA256 password hashing.
"""
...
from clara.database.models import (
    AIUsageRecord,
    FileRecord,
    Message,
    Room,
...
_ITERATIONS = settings.security.password_iterations
...
class ClaraDB:
    """Thread-safe SQLite backend for all CLARA data."""

    def __init__(self, path: Optional[Path] = None) -> None:
        self._path = str(path or settings.database.sqlite_path)
    ...
    def _ensure_default_room(self) -> None:
        if self.conn.execute("SELECT 1 FROM rooms WHERE name='general'").fetchone() is None:
            self.conn.execute(
    ...
    def authenticate(self, username: str, password: str) -> Optional[User]:
        ...
        if not user or user.banned or not verify_password(password, user.password_hash, user.salt):
            return None
        return user
    ...
    def leave_room(self, room_name: str, username: str) -> None:
        self.conn.execute("DELETE FROM room_members WHERE room_name=? AND username=?", (room_name, username))
        self.conn.commit()

    def get_room_members(self, room_name: str) -> list[str]:
        rows = self.conn.execute(
            "SELECT username FROM room_members WHERE room_name=? ORDER BY username", (room_name,)
        ).fetchall()
    ...
    def delete_message(self, msg_id: int, sender: str) -> bool:
        cur = self.conn.execute("UPDATE messages SET deleted=1 WHERE id=? AND sender=?", (msg_id, sender))
        self.conn.commit()
        return cur.rowcount > 0

    def search_messages(self, room: str, query: str, limit: int = 50) -> list[Message]:
        rows = self.conn.execute(
            "SELECT * FROM messages WHERE room=? AND content LIKE ? AND deleted=0 "
    ...
    def get_recent_messages(self, room: str, limit: int = 50) -> list[Message]:
        rows = self.conn.execute(
            "SELECT * FROM messages WHERE room=? AND deleted=0 AND recipient='' "
            "ORDER BY timestamp DESC LIMIT ?",
            (room, limit),
        ).fetchall()
    ...
    def get_dm_history(self, user1: str, user2: str, limit: int = 50) -> list[Message]:
        rows = self.conn.execute(
            "SELECT * FROM messages WHERE deleted=0 AND recipient!='' "
    ...
    def list_files(self, room: str = "") -> list[FileRecord]:
        if room:
            rows = self.conn.execute("SELECT * FROM files WHERE room=? ORDER BY uploaded_at DESC", (room,)).fetchall()
        else:
    ...
    def get_ai_usage(self, username: str) -> dict:
        row = self.conn.execute(
            ...
            "COALESCE(SUM(cost),0.0) as total_cost, COUNT(*) as requests "
            "FROM ai_usage WHERE username=?",
            (username,),
        ).fetchone()
    ...
    def create_voice_session(self, caller: str, callee: str = "", room: str = "",
                             session_type: str = "p2p") -> int:
        cur = self.conn.execute(
    ...
    def end_voice_session(self, session_id: int) -> None:
        self.conn.execute("UPDATE voice_sessions SET ended_at=? WHERE id=?", (time.time(), session_id))
        self.conn.commit()
